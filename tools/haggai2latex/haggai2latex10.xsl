...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="XMLBIBLE">
    ...
    <xsl:text>% This file was generated by haggai2latex10.xsl, which is free software licensed under the GNU Affero General Public License 3 or any later version (see https://github.com/free-scriptures/free-scriptures/ and http://www.free-scriptures.org).&#xA;</xsl:text>
    <xsl:text>&#xA;</xsl:text>
    <xsl:text>\usepackage[utf8]{inputenc}&#xA;</xsl:text>
    <xsl:text>\usepackage[a6paper,left=1.6cm,right=1cm,top=2cm,bottom=0.25cm]{geometry}&#xA;</xsl:text>
...
{%
  \zsavepos{#1}%
    \ifnum#2&lt;10%
      \hbox to 0pt{\hskip\dimexpr-\zposx{#1}sp +1.18cm \relax#2}%
    \else%
      \ifnum#2&gt;99%
        \hbox to 0pt{\hskip\dimexpr-\zposx{#1}sp +0.87cm \relax#2}%
      \else
        \hbox to 0pt{\hskip\dimexpr-\zposx{#1}sp +1.02cm \relax#2}%
      \fi
    \fi
}%
&#xA;</xsl:text>
...
&#xA;</xsl:text>
  ...
  <xsl:template match="CHAPTER">
    ...
    <xsl:text> </xsl:text><xsl:value-of select="./@cnumber"/><xsl:text>}</xsl:text>
    <xsl:text>\lettrine[lraise=0.10,findent=5pt,nindent=0pt]{</xsl:text>
  ...
  <xsl:template match="NOTE">
    <!-- The small DIN A6 version ignores variant notes in order to save paper
         space, this feature could be reserved for the versions which are
         typeset on larger paper dimensions. -->
    <xsl:if test="not(@type='variant')">
