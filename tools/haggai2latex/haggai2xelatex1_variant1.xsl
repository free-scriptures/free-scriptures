...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="XMLBIBLE">
    ...
    <xsl:text>% This file was generated by haggai2xelatex1_variant1.xsl, which is free software licensed under the GNU Affero General Public License 3 or any later version (see https://github.com/free-scriptures/free-scriptures/ and http://www.free-scriptures.org).&#xA;</xsl:text>
    <xsl:text>&#xA;</xsl:text>
  ...
  <xsl:template match="CHAPTER">
    ...
    <xsl:choose>
      <xsl:when test="../@bname or ../@bsname">
        ...
        <xsl:text> </xsl:text><xsl:value-of select="./@cnumber"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:text>Kapitel </xsl:text><xsl:value-of select="./@cnumber"/>
        <xsl:text>}{</xsl:text>
        <xsl:text>Kapitel </xsl:text><xsl:value-of select="./@cnumber"/>
      </xsl:otherwise>
    </xsl:choose>
