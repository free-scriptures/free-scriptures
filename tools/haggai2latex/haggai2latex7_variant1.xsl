...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="XMLBIBLE">
    <xsl:text>\documentclass[a5paper,8pt]{extarticle}&#xA;</xsl:text>
    <xsl:text>% This file was generated by haggai2latex7_variant1.xsl, which is free software licensed under the GNU Affero General Public License 3 or any later version (see https://github.com/free-scriptures/free-scriptures/ and http://www.free-scriptures.org).&#xA;</xsl:text>
    <xsl:text>&#xA;</xsl:text>
    <xsl:text>\usepackage[utf8]{inputenc}&#xA;</xsl:text>
    <xsl:text>\usepackage[paperwidth=5.5in,paperheight=8.5in,width=4in,height=6.5in,top=1.1in]{geometry}&#xA;</xsl:text>
    ...
    <xsl:text>% bigfoot will break register accuracy.&#xA;</xsl:text>
    <xsl:text>%\usepackage{bigfoot}&#xA;</xsl:text>
    <xsl:text>% For bigfoot.&#xA;</xsl:text>
...
\AddToShipoutPicture{%
  \begingroup
    \linethickness{0.5pt}%
    \setlength{\@tempdima}{\textwidth+0.2in}%
    \setlength{\@tempdimb}{\textheight+0.2in}%
    \put(\LenToUnit{0.65in},\LenToUnit{1.04in}){%
...
{%
  ...
  \ifnum13769490>\number\zposx{#1}%
    \ifnum#2&lt;10%
      \hbox to 0pt{\hskip\dimexpr-\zposx{#1}sp +0.5in \relax#2}%
    \else%
      \ifnum#2&gt;99%
        \hbox to 0pt{\hskip\dimexpr-\zposx{#1}sp +0.38in \relax#2}%
      \else
        \hbox to 0pt{\hskip\dimexpr-\zposx{#1}sp +0.44in \relax#2}%
      \fi
    \fi
  \else%
    \hbox to 0pt{\hskip\dimexpr-\zposx{#1}sp +4.933in \relax#2}%
  \fi%
}%
&#xA;</xsl:text>
    <!-- fancyhdr -->
