 ...
 */
/**
 * @file $/workflows/osis2pdf1/osis2pdf1.java
 * @brief Workflow to automatically process an OSIS file to PDF.
...
public class osis2pdf1
{
    public static void main(String args[])
    {
        System.out.print("osis2pdf1 workflow  Copyright (C) 2015  Stephan Kreutzer\n" +
        ...
        String programPath = osis2pdf1.class.getProtectionDomain().getCodeSource().getLocation().getFile();
        ...
        if (osisFile == null)
        {
            System.out.println("osis2pdf1 workflow: No input OSIS file specified.");
            System.exit(-1);
        }

        if (osisFile.exists() != true)
        {
            System.out.print("osis2pdf1 workflow: OSIS file '" + osisFile.getAbsolutePath() + "' doesn't exist.\n");
            System.exit(-1);
        }

        if (osisFile.isFile() != true)
        {
            System.out.print("osis2pdf1 workflow: Path '" + osisFile.getAbsolutePath() + "' isn't a file.\n");
            System.exit(-1);
        }

        if (osisFile.canRead() != true)
        {
            System.out.print("osis2pdf1 workflow: OSIS file '" + osisFile.getAbsolutePath() + "' isn't readable.\n");
            System.exit(-1);
        }
        ...
        if (tempDirectory.exists() != true)
        {
            ...
            {
                System.out.print("osis2pdf1 workflow: Can't create temp directory '" + tempDirectory.getAbsolutePath() + "'.\n");
                System.exit(-1);
            }
        }
        else
        {
            ...
            {
                System.out.print("osis2pdf1 workflow: Temp directory path '" + tempDirectory.getAbsolutePath() + "' exists, but isn't a directory.\n");
                System.exit(-1);
            }
        }
        ...
        {
            System.out.print("osis2pdf1 workflow: Haggai XML file '" + haggaiFile.getAbsolutePath() + "' doesn't exist, but should by now.\n");
            System.exit(-1);
        }

        if (haggaiFile.isFile() != true)
        {
            System.out.print("osis2pdf1 workflow: Path '" + haggaiFile.getAbsolutePath() + "' isn't a file.\n");
            System.exit(-1);
        }

        if (haggaiFile.canRead() != true)
        {
            System.out.print("osis2pdf1 workflow: Haggai XML file '" + haggaiFile.getAbsolutePath() + "' isn't readable.\n");
            System.exit(-1);
        }
        ...
        if (backend.equalsIgnoreCase("pdflatex") == true)
        {
            builder = null;
            ...
            if (args.length >= 3)
            {
                ...
                if (args.length >= 5)
                {
                    builder = new ProcessBuilder("java", "haggai2pdf1", haggaiFile.getAbsolutePath(), args[2], args[3], args[4]);
                }
                else if (args.length == 4)
                {
                    builder = new ProcessBuilder("java", "haggai2pdf1", haggaiFile.getAbsolutePath(), args[2], args[3]);
                }
                else if (args.length == 3)
                {
                    builder = new ProcessBuilder("java", "haggai2pdf1", haggaiFile.getAbsolutePath(), args[2]);
                }
            }
            else
            {
                builder = new ProcessBuilder("java", "haggai2pdf1", haggaiFile.getAbsolutePath());
            }

            builder.directory(new File(programPath + ".." + File.separator + "haggai2pdf1"));
            builder.redirectErrorStream(true);
